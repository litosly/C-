...
int main()
{
	...
	int n,m,c,i,j,a,b,h,x,y,ans;
	while (scanf("%d%d%d",&n,&m,&c)!=EOF){
		/*memset(A,0,sizeof(A));
		for (i=1;i<=n;++i)
			for (j=1;j<=m;++j)scanf("%d",&A[i][j]);*/
		len=0;Build(++len,0,m,0,n);
		for (i=1;i<=c;++i){
			scanf("%d%d%d%d%d",&a,&b,&h,&x,&y);
			ans=Qmax(1,0,m,0,n,y,y+b,x,x+a);
			Cover(1,0,m,0,n,y,y+b,x,x+a,ans+h);
		}
		printf("%d\n",Qmax(1,0,m,0,n,0,m,0,n));
	}
	system("pause");for (;;);
	return 0;
}
